...
exports.FormPublicationService = void 0;
...
class FormPublicationService {
    ...
    async getPublicationSettings(formId) {
        try {
            const defaultSettings = {
                formId,
                visibility: form_sharing_models_1.FormVisibility.PRIVATE,
                isPublished: false,
            ...
            return form_sharing_models_1.FormPublicationSettingsSchema.parse(defaultSettings);
        }
        catch (error) {
    ...
    async updatePublicationSettings(formId, settings) {
        try {
            ...
            return form_sharing_models_1.FormPublicationSettingsSchema.parse(updatedSettings);
        }
        catch (error) {
            throw new Error(`Failed to update publication settings for form ${formId}: ${error}`);
        }
    }
    async publishForm(formId, visibility = form_sharing_models_1.FormVisibility.PUBLIC, options) {
        try {
            const settings = {
                visibility,
                isPublished: true,
    ...
    async generateEmbedCode(formId, settings) {
        try {
            const defaultSettings = {
                formId,
                theme: form_sharing_models_1.EmbedTheme.LIGHT,
                autoHeight: true,
                width: '100%',
                borderRadius: 8,
                hideHeader: false,
            ...
            const embedSettings = form_sharing_models_1.EmbedCodeSettingsSchema.parse({
                ...defaultSettings,
                ...settings
            });
...
></iframe>`;
    ...
    buildEmbedParams(settings) {
        ...
        if (settings.theme !== form_sharing_models_1.EmbedTheme.LIGHT)
    ...
    async generateShareLink(formId, options) {
        try {
            ...
            return form_sharing_models_1.ShareLinkSchema.parse(shareLink);
        }
        catch (error) {
    ...
    async updateShareLink(linkId, updates) {
        try {
            const existingLink = {
                id: linkId,
                formId: 'mock-form-id',
                type: form_sharing_models_1.ShareLinkType.STANDARD,
            ...
            return form_sharing_models_1.ShareLinkSchema.parse(updatedLink);
        }
        catch (error) {
    ...
    async getFormSharingStats(formId) {
        try {
            ...
            return form_sharing_models_1.FormSharingStatsSchema.parse(stats);
        }
        catch (error) {
    ...
    async performBulkOperation(operation) {
        try {
            const validatedOperation = form_sharing_models_1.BulkSharingOperationSchema.parse({
                ...operation,
                status: 'in_progress',
...
exports.FormPublicationService = FormPublicationService;
