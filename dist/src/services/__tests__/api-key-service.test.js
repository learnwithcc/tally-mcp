...
describe('ApiKeyService', () => {
    ...
    describe('rotateApiKey', () => {
        ...
        it('should rotate API key and return new key', async () => {
            ...
            expect(rotated?.id).toBe(testKey.id);
            expect(rotated?.name).toBe(testKey.name);
            expect(rotated?.usageCount).toBe(0);
        });
...
describe('CryptoUtils', () => {
    ...
    describe('generateApiKey', () => {
        it('should generate API key with default options', () => {
            ...
            expect(key).toMatch(/_[A-Za-z0-9_-]{8}$/);
        });
        it('should generate API key with custom options', () => {
            ...
            expect(key).not.toMatch(/_[A-Za-z0-9_-]{8}$/);
        });
        it('should throw error for invalid length', () => {
            expect(() => {
                CryptoUtils.generateApiKey({ length: 8 });
            }).toThrow();
            expect(() => {
                CryptoUtils.generateApiKey({ length: 128 });
            }).toThrow();
        });
    });
    describe('validateKeyFormat', () => {
        ...
        it('should reject invalid key format', () => {
            ...
            expect(CryptoUtils.validateKeyFormat('tally_validkey')).toBe(false);
