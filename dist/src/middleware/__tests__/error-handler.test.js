import { globalErrorHandler, asyncErrorHandler, notFoundHandler, setupProcessErrorHandlers } from '../error-handler';
import { ErrorCategory } from '../../server';
...
describe('Error Handler Middleware', () => {
    ...
    describe('globalErrorHandler', () => {
        ...
        it('should handle structured errors', () => {
            process.env.NODE_ENV = 'production';
            const req = createMockRequest({ requestId: 'test-123' });
            const res = createMockResponse();
            const structuredError = new Error('Validation failed');
    ...
    describe('notFoundHandler', () => {
        it('should create and pass a 404 structured error', () => {
            const req = createMockRequest({ path: '/non-existent', requestId: 'req-123' });
            const res = createMockResponse();
    ...
    describe('setupProcessErrorHandlers', () => {
        ...
        it('should handle unhandled rejection with logger', () => {
            ...
            expect(mockLogger).toHaveBeenCalledWith('error', 'Unhandled Promise Rejection: Error: Test rejection', testError);
        });
        it('should handle unhandled rejection without logger', () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith('Unhandled Promise Rejection: Test rejection reason', 'Test rejection reason');
        });
        it('should handle uncaught exception with logger', () => {
            ...
            expect(mockLogger).toHaveBeenCalledWith('fatal', 'Uncaught Exception: Test exception', testError);
        ...
        it('should handle uncaught exception without logger', () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith('Uncaught Exception: Test exception', testError);
            expect(processExitSpy).toHaveBeenCalledWith(1);
            processExitSpy.mockRestore();
        });
    });
