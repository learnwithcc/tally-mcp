...
describe('Logging and Monitoring Integration Tests', () => {
    ...
    describe('Workflow Execution Logging', () => {
        it('should produce detailed logs for successful workflow execution', async () => {
            ...
            const logs = testSuite.executionLogs;
            expect(logs.some((log) => log.stage === 'start')).toBe(true);
            expect(logs.some((log) => log.stage === 'complete')).toBe(true);
            expect(logs.some((log) => log.stage === 'validation')).toBe(true);
            expect(logs.some((log) => log.workflow === 'logging-test-success')).toBe(true);
        });
        it('should produce detailed logs for failed workflow execution', async () => {
            ...
            await testSuite.simulateFailureScenario('logging-test-failure', 'server-error', 'form-creation', request);
            const stats = testSuite.getExecutionStats();
            expect(stats.totalLogs).toBeGreaterThan(0);
            expect(stats.errors).toBeGreaterThan(0);
            const logs = testSuite.executionLogs;
            expect(logs.some((log) => log.stage === 'error')).toBe(true);
            expect(logs.some((log) => log.workflow === 'logging-test-failure')).toBe(true);
            expect(logs.some((log) => log.data && log.data.error)).toBe(true);
        });
        it('should log data transformation stages correctly', async () => {
            ...
            await testSuite.executeWorkflow('log-transformation-test', request, ['mcp-request', 'form-retrieval', 'nlp-processing', 'tally-api-call', 'response-transformation']);
            const logs = testSuite.executionLogs;
            const validationLog = logs.find((log) => log.stage === 'validation');
        ...
        it('should log performance metrics for each workflow', async () => {
            ...
            const logs = testSuite.executionLogs;
            const completeLog = logs.find((log) => log.stage === 'complete');
    ...
    describe('Monitoring Outputs Validation', () => {
        ...
        it('should provide comprehensive execution statistics', async () => {
            ...
            await testSuite.simulateFailureScenario('stats-failure-1', 'timeout', 'submission-retrieval', testData.submissionRequest('stats-form-3'));
            const stats = testSuite.getExecutionStats();
            expect(stats.totalLogs).toBeGreaterThan(0);
            expect(stats.uniqueWorkflows).toBe(3);
            expect(stats.errors).toBe(1);
            const successfulWorkflows = new Set(testSuite.executionLogs
                .filter((log) => log.stage === 'complete')
                .map((log) => log.workflow));
    ...
    describe('Log Content and Formatting', () => {
        ...
        it('should include request and response data in logs where appropriate', async () => {
            ...
            const logs = testSuite.executionLogs;
            const startLog = logs.find((log) => log.stage === 'start' && log.workflow === 'log-data-inclusion-test');
            const completeLog = logs.find((log) => log.stage === 'complete' && log.workflow === 'log-data-inclusion-test');
        ...
        it('should serialize error objects correctly in logs', async () => {
            const testData = testSuite.createTestData();
            await testSuite.simulateFailureScenario('log-error-serialization-test', 'server-error', 'form-creation', testData.formCreationRequest('Error Serialization'));
            const logs = testSuite.executionLogs;
            const errorLog = logs.find((log) => log.stage === 'error' && log.workflow === 'log-error-serialization-test');
...
//# sourceMappingURL=workflow-logging-monitoring.test.js.map